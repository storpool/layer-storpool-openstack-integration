...
def config_changed():
    ...
    rdebug(
        "and we do{xnot} have a storpool_version setting".format(
            xnot=" not" if spver is None else ""
        )
    )
    sposiver = config.get("storpool_openstack_version", None)
    rdebug(
        "and we do{xnot} have a storpool_openstack_version setting".format(
            xnot=" not" if sposiver is None else ""
        )
    )
...
def install_package():
    ...
    """
    rdebug(
        "the OpenStack integration repo has become available and "
...
def enable_and_start():
    ...
    spstatus.npset(
        "maintenance",
        "installing the OpenStack integration into " "the running containers",
    )
    ...
    for comp in openstack_components:
        ...
        if res["res"] == 0:
            ...
            rdebug(
                "    - running sp-openstack install {comp}".format(comp=comp)
            )
            res = sputils.exec(
                safe_path
                + [
            ...
            if res["res"] != 0:
                raise spe(
                    "Could not install the StorPool OpenStack "
    ...
    if check_groups:
        ...
        res = sputils.exec(
            safe_path
            + ["sp-openstack", "-T", txn.module_name(), "--", "groups"]
            + check_groups
        )
        if res["res"] != 0:
            raise spe(
                "Could not setup the StorPool OpenStack "
        ...
        for comp in check_groups:
            ...
            if len(lines) != 1:
                raise spe(
                    ...
                    'the "disk" group: unexpected "id -Gn" output: "'
                    "{lines}".format(comp=comp, lines=lines)
                )
            groups = lines[0].split(" ")
            if "disk" not in lines[0].split(" "):
                rdebug(
                    'Adding the {comp} service account to the "disk" '
                    "group".format(comp=comp)
                )
                groups.append("disk")
                subprocess.check_call(
                    ["usermod", "-G", ",".join(groups), "--", comp]
                )
                restart[comp] = True
    ...
    for comp in sorted(restart.keys()):
        ...
        if svc is None:
            ...
            continue
        rdebug(
            "Restarting the {svc} service (errors will be ignored)".format(
                svc=svc
            )
        )
