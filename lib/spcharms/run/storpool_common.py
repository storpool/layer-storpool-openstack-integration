...
def install_package():
    ...
    """
    rdebug(
        "the common repo has become available and "
    ...
    with open("/proc/cmdline", mode="r") as f:
        ...
        if missing:
            if sputils.bypassed("kernel_parameters"):
                ...
                raise spe(
                    "Missing kernel parameters: {missing}".format(
    ...
    for pattern in ("storpool-*", "kmod-storpool-*", "python-storpool-*"):
        try:
            rdebug(
                "obtaining information about the {pat} packages".format(
                    pat=pattern
                ),
                cond="run-common",
            )
            ...
            rdebug(
                "got {count} raw lines".format(count=len(lines)),
                cond="run-common",
            )
            ...
            for line in lines:
                ...
                if len(fields) != 2:
                    rdebug(
                        "- weird line with {count} fields: {line}".format(
                            count=len(fields), line=line
                        )
                    )
                    continue
                (pkg, status) = fields
                rdebug(
                    "- package {pkg} status {st}".format(pkg=pkg, st=status),
                    cond="run-common",
                )
            ...
            rdebug(
                "could not query the {pat} packages: {e}".format(
                    pat=pattern, e=e
                )
            )

    rdebug(
        "{count} packages to install/upgrade".format(
...
def parse_cgroup_slice_size():
    ...
    if ks != exp:
        raise sperror.StorPoolException(
            "Invalid cgroup_slice_size keys: must have exactly {exp}".format(
                exp=exp
            )
        )
    ...
    with open("/proc/cpuinfo", mode="r") as f:
        ...
        all_cpus = sorted(
            map(
                ...
                filter(
                    lambda lst: lst and lst[0] == "processor",
        ...
        while True:
            ...
            if words[0] == "MemTotal:":
                ...
                if unit.startswith("K"):
                    ...
                    raise spe(
                        'Could not parse the "{u}" unit for MemTotal '
        ...
        for fname in files:
            ...
            if fname in (
                ...
                with tempfile.NamedTemporaryFile(
                    ...
                    rdebug(
                        "- generating {tempf} for {dst}".format(
                ...
                if os.path.isfile(dst):
                    ...
                    try:
                        ...
                        rdebug(
                            "COULD NOT remove {dst}: {e}".format(dst=dst, e=e)
                        )
                else:
