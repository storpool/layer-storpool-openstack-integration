...
STORPOOL_PRESENCE_SCHEMA_1_0 = {
    "format": {"version": {"major": int, "minor": int}},
    "generation": int,
    "nodes": {
        "*": {
            "generation": int,
...
def validate_dict(value, schema):
            ...
            if not isinstance(v, t):
                raise ValidationError(
                    ...
                    "not a {t}, {vt} instead".format(
                        t=t.__name__, vt=type(v).__name__
                    ),
                )
        else:
    ...
    if extra:
        raise ValidationError(
            None, "extra keys: {lst}".format(lst=sorted(extra))
        )
...
def presence_send(relname, data):
    for name in hookenv.relation_ids(relname) or []:
        try:
            ...
            hookenv.log(
                "Could not send the presence data out on "
